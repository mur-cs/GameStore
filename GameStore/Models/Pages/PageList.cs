...
namespace GameStore.Models.Pages
{
    public class PagedList<T>:List<T>
    {
        ...
        private static IQueryable<T> Order(IQueryable<T> query, string propertyName, bool desc)
        {
            ...
            var result = typeof(Queryable).GetMethods().Single(x => x.Name==(desc ? "OrderByDescending" : "OrderBy") && x.IsGenericMethodDefinition && x.GetGenericArguments().Length==2 && x.GetParameters().Length==2).MakeGenericMethod(typeof(T), source.Type).Invoke(null, new object[] { query, lambda }) as IQueryable<T>; 
			return typeof(Queryable).GetMethods().Single(x => x.Name==(desc ? "OrderByDescending" : "OrderBy") && x.IsGenericMethodDefinition && x.GetGenericArguments().Length==2 && x.GetParameters().Length==2).MakeGenericMethod(typeof(T), source.Type).Invoke(null, new object[] { query, lambda }) as IQueryable<T>;
		}
    }
}
